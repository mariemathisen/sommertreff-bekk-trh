  ...
  return (
    <div className="min-h-screen bg-[#d6e8f5] px-4 py-10">
      <div className="max-w-3xl mx-auto space-y-10">
        ...
        <section className="space-y-3">
          <h2 className="text-lg font-semibold text-zinc-800">Aktivitetsbeskrivelser</h2>
          <div className="space-y-3">
            {[
              { title: 'Logikkolympics', room: 'Videoloftet', body: 'Hvordan tenker man logisk når tiden går og laget har tre ulike teorier samtidig? Hvor raskt klarer man å se mønstre, finne system og lande et svar før de andre lagene? I Logikkolympics venter tre oppgaver hvor samarbeid, resonnering og litt konkurranseinstinkt fort blir viktigere enn man skulle tro.' },
              { title: 'Sommelierlekene', room: 'Sofaloftet', body: 'Hvor mye av vinsmaking handler egentlig om kunnskap, og hvor mye handler om å si ting med nok selvtillit? I Sommelierlekene blir det fort tydelig hvem som faktisk kan vin og hvem som bare kan snakke om den.' },
              { title: 'Kongen befaler', room: 'Videoloftet', body: 'Hvor kreativ blir man egentlig når oppgavene gir null mening ved første gjennomlesning? Og hvor langt er man villig til å strekke komfortsonen for noen ekstra poeng?' },
              { title: 'MVP før midnatt', room: 'Hele kvelden', body: 'Kan man selge hva som helst med riktig nettside og nok selvtillit? Hvor langt kommer man med sterke vibber, tvilsomme buzzwords og et godt design? I MVP før midnatt får dere tildelt et produkt og skal lage den mest kreative nettsiden dere klarer før alt pitchepresenteres for resten på slutten av kvelden.' },
            ].map((activity) => (
              <div key={activity.title} className="rounded-lg border border-zinc-200 bg-white p-4">
                <div className="flex items-center gap-2">
                  <h3 className="text-sm font-semibold text-zinc-800">{activity.title}</h3>
                  <span className="text-xs text-zinc-400">{activity.room}</span>
                </div>
                <p className="mt-1 text-sm text-zinc-600">{activity.body}</p>
              </div>
            ))}
          </div>
        </section>
